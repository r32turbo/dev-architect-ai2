...
# ✅ NOW WE CAN IMPORT PROMPTS (which imports reusableagents)
try:
    from .prompts import SYSTEM_ARCHITECT_PROMPT
except ImportError:
    from prompts import SYSTEM_ARCHITECT_PROMPT
...
# ✅ CHUNKING UTILITY
def chunk_text(text: str, chunk_size: int = 32000, overlap: int = 500) -> list[str]:
    """
    ...
    ----------
    text       : text to chunk
    chunk_size : size of each chunk (default 32000, can be overridden by CHUNK_SIZE env var)
...
def build_agent(context: "AgentContext | None" = None):
    ...
    # ✅ Use the PromptBuilder from prompts.py
    prompt_builder = SYSTEM_ARCHITECT_PROMPT
...
>>>>>>> 1a959f420456e51010b04b7ddcbcb43d3b28eb36
    ...
    """
    Run the System Architecture Agent.
    
    Parameters
    ----------
    user_input       : user request (optional if in context)
    requirement_doc  : requirement document (optional if in context or database)
    context          : AgentContext with state
    
    Returns
    -------
    str : generated architecture document
    """
    load_environment()
...
<<<<<<< HEAD
    ...
    if context and hasattr(context, 'state') and context.state:
        ...
        if not requirement_doc and 'requirement_doc' in context.state:
            requirement_doc = context.state.get('requirement_doc')

    # ✅ FETCH FROM STATE IF STILL NOT PROVIDED
    if not str(user_input or "").strip():
        state = ArchitectState()
        user_input = state.get_input()
