...
# If app.py is at /deps/course-architect/src/agent/app.py, then ../frontend/dist resolves to /deps/frontend/dist.
#COPY --from=frontend-builder /app/frontend/dist /deps/frontend/dist
# -- End of copying built frontend --

# -- Adding local package . --
ADD course-architect/ /deps/course-architect
...
RUN cd /deps/course-architect && \
    PYTHONDONTWRITEBYTECODE=1 UV_SYSTEM_PYTHON=1 uv pip install --system -c /api/constraints.txt -e .
# -- End of local dependencies install --
ENV LANGGRAPH_HTTP='{"app": "/deps/course-architect/src/agent/app.py:app"}'
ENV LANGSERVE_GRAPHS='{"agent": "/deps/course-architect/src/agent/graph.py:graph"}'
